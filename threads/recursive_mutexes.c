...
// With recursive mutexes, even if we lock the same mutex for same thread multiple times, it doesn't hit deadlock state.
// Only thing is that, we have unlock the mutex same number of times we locked it.
